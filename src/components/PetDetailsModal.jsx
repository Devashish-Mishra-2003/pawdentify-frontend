...
export default function PetDetailsModal({ isOpen, onClose, pet, onAddNote, onDeleteNote }) {
  ...
  const categories = [
    { value: 'health', label: t('dashboard.pets.petDetails.categories.health'), color: '#F07E7E', icon: '🏥' },
  ...
  const calculateAge = (birthday) => {
    if (!birthday) return t('common.unknown');
      ...
      >
        ...
        >
          ...
          <div className="relative h-96 flex-shrink-0">
             ...
             <button 
               onClick={onClose}
               className="absolute top-8 right-8 w-12 h-12 rounded-full bg-black/50 backdrop-blur-md border border-white/10 flex items-center justify-center text-white hover:bg-[#F07E7E] transition-all"
             >
             ...
             <div className="absolute bottom-10 left-10 right-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                   <span className="font-handwriting text-4xl text-[#30A7DB] mb-2 block">Our Companion</span>
                   <h2 className="text-5xl md:text-7xl font-black text-white leading-[0.8] tracking-tighter uppercase">{pet.name}</h2>
                </div>
                <div className="flex gap-4">
                   <div className="bg-white/10 backdrop-blur-md border border-white/10 rounded-3xl p-4 min-w-[120px]">
                      <span className="block text-[8px] font-black uppercase text-white/50 tracking-widest mb-1">{t('dashboard.pets.petBreed')}</span>
                      <span className="text-white font-bold">{pet.breed}</span>
                   </div>
                   <div className="bg-[#30A7DB]/20 backdrop-blur-md border border-[#30A7DB]/20 rounded-3xl p-4 min-w-[120px]">
                      <span className="block text-[8px] font-black uppercase text-[#30A7DB] tracking-widest mb-1">{t('dashboard.pets.age')}</span>
                      <span className="text-white font-bold">{calculateAge(pet.birthday)}</span>
                   </div>
                </div>
             </div>
          </div>
            ...
            <div className="flex items-center gap-3 overflow-x-auto no-scrollbar pb-2">
               ...
               {categories.map(cat => (
                  <button 
                    ...
                    className={`px-6 py-3 rounded-full text-xs font-black uppercase tracking-widest border-2 transition-all whitespace-nowrap flex items-center gap-2 ${filterCategory === cat.value ? 'shadow-lg border-current' : 'bg-gray-50 dark:bg-white/5 border-gray-100 dark:border-white/10 text-gray-400 hover:border-gray-200'}`}
                    style={{ 
            ...
            <div className="space-y-6 mb-8 min-h-[200px]">
              <AnimatePresence mode="popLayout">
                {filteredNotes && filteredNotes.length > 0 ? (
                  ...
                  <div className="bento-card border-dashed border-2 border-gray-100 dark:border-white/10 bg-gray-50/50 dark:bg-white/5 py-20 text-center">
                     <div className="w-20 h-20 bg-gray-100 dark:bg-white/5 rounded-full mx-auto mb-6 flex items-center justify-center text-3xl opacity-20">📭</div>
                     <p className="text-gray-400 font-bold uppercase tracking-widest text-xs">{filterCategory === 'all' ? 'Your journal is empty' : `No ${filterCategory} logs found`}</p>
                  </div>
                )}
              </AnimatePresence>
            </div>
          ...
          <div className="p-10 pt-0 flex-shrink-0">
             <button
               onClick={onClose}
               className="w-full pill-button bg-gray-100 dark:bg-white/5 text-gray-400 dark:text-gray-500 hover:text-black dark:hover:text-white border-0 shadow-none py-6 font-black uppercase"
             >
               Return to Sanctuary
             </button>
          </div>
